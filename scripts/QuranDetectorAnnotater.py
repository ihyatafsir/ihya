...
class qMatcherAnnotater():
    ...
    def updateResults(self, k, memAya, memVs, mem, result, er, cv, end):
        if k.name not in memAya:
            return True # Not in memory, so treat as new
        ...
        if prev == memVs[idx]:
            ...
            # Instead of clearing everything, we remove the ones we just updated
            # or ones that are no longer relevant. 
            # Original code cleared EVERYTHING if a sequence was found.
            # This caused crashes in the overlap loop.
            
            # We use a try-except to be extra safe
            try:
                memAya.pop(idx)
                memVs.pop(idx)
                mem.pop(idx)
